...
public class FacebookService extends AbstractSocialService<Facebook> {
	...
	public UserEntity createUser(Connection<?> connection) {
		...
		return new UserEntity()
			.setUsername(StringUtils.join(profile.getFirstName(), profile.getLastName(), randomUsername))
			.setEmail(StringUtils.isBlank(profile.getEmail()) ? profile.getEmail() : StringUtils.join(profile.getId(), "@facebook.com"))
